...
'''
这个是所有点都要算的，暴力算法，直接算出buffer内所有POI点距离出发点的距离。
如果能解决for循环慢的问题，效率可能会比compute_walkscore_1的算法快。
这个算法暂时没有使用，太慢了
'''
def Compute_Walkscore_Road(start_points_info, weight_tables:dict, MultiType_poi_points_geo:dict, road_geo, Multi_roads_geos, G):          
    ...
    buffer_3000 = road_geo.Buffer(3000)
    roads_in_buffer = Multi_roads_geos.Intersection(buffer_3000)
    ...
    for start_pointInfo in start_points_info:    
        ...
        for poi_type in MultiType_poi_points_geo.keys():
            ...
            if count_in_buffer == 0:
                continue
            print(count_in_buffer)
            for i in range(count_in_buffer):
                temp_geo_poi = poi_points_geo_in_buffer[poi_type].GetGeometryRef(i)
                distance = net_Distance(G, road_geo, Multi_roads_geos, temp_geo_start, temp_geo_poi)
